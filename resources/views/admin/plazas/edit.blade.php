...
@section('content')
    <div class="container mx-auto p-4">
        <h1 class="text-2xl font-bold mb-4">Editar Plaza</h1>
        ...
        <form action="{{ route('admin.plazas.update', $plaza) }}" method="POST" class="bg-white p-4 rounded shadow">
            @csrf
            @method('PUT')
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="area" class="w-full border p-2 rounded"
                    value="{{ old('area', $plaza->area) }}" required>
            </div>
            <div class="mb-4">
                <label class="block mb-1 font-bold">Período académico</label>
                <input type="text" name="periodo" class="w-full border p-2 rounded"
                    value="{{ old('periodo', $plaza->periodo) }}" required>
            </div>
            <div class="mb-4">
                <label class="block mb-1 font-bold">Vacantes</label>
                <input type="number" name="vacantes" class="w-full border p-2 rounded"
                    value="{{ old('vacantes', $plaza->vacantes) }}" min="1" required>
            </div>
            <div class="mb-4">
                <label class="block mb-1 font-bold">Requisitos</label>
                <textarea name="requisitos" class="w-full border p-2 rounded" rows="4">{{ old('requisitos', $plaza->requisitos) }}</textarea>
            </div>
