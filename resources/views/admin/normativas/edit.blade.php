...
@section('content')
    <div class="container mx-auto p-4 max-w-2xl">
        ...
        <form action="{{ route('admin.normativas.update', $normativa) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <textarea id="contenido" name="contenido" rows="6" class="w-full border rounded p-2 mb-4" required>{{ old('contenido') ?? $normativa->contenido }}</textarea>
            @error('contenido')
                <p class="text-red-600">{{ $message }}</p>
            @enderror
