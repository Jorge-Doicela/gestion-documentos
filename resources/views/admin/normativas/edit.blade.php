...
@section('content')
    <div class="container mx-auto p-4 max-w-2xl">
        ...
        <form action="{{ route('admin.normativas.update', $normativa) }}" method="POST" novalidate>
            @csrf
            @method('PUT')
                ...
                <select id="tipo_documento_id" name="tipo_documento_id"
                    ...
                    @foreach ($tipos as $tipo)
                        <option value="{{ $tipo->id }}"
                            {{ (old('tipo_documento_id') ?? $normativa->tipo_documento_id) == $tipo->id ? 'selected' : '' }}>
                            {{ $tipo->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <textarea id="contenido" name="contenido" rows="6"
                    class="w-full border rounded p-2 @error('contenido') border-red-600 @enderror" aria-describedby="contenidoError">{{ old('contenido') ?? $normativa->contenido }}</textarea>
                @error('contenido')
